...
public class CompanyController {
    ...
    public BaseResponse<List<TcCompany>> selectCompany(String uId) {
        BaseResponse<List<TcCompany>> baseResponse = new BaseResponse<>();
        try {
            if (uId == null || uId.equals("")) {
                baseResponse.setInfo("参数错误，查询失败");
                return baseResponse;
            }
            List<TcCompany> tcCompanies = companyService.selectAllCompany(Integer.parseInt(uId));
            baseResponse.setCode(1);
            baseResponse.setData(tcCompanies);
        } catch (Exception e) {
            baseResponse.setInfo("参数错误，查询失败");
        }
        return baseResponse;
